...
- **Journey rail goes horizontal on mobile (steps squashed into a row)** – almost always caused by an inline `style="grid-template-columns:repeat(N,1fr)"` on `.journey-steps`. Inline styles beat the mobile media query in `site.css` that forces `grid-template-columns:1fr` for the vertical timeline. Use the `.journey-steps--3` class (or add a similar class for other column counts) instead of an inline style. The mobile rule uses `!important` to defend against this regression – keep it. On mobile the journey rail must be vertical: progress bar on the left, markers on the left, content stacked to the right – matching the desktop visual structure rotated 90°.
